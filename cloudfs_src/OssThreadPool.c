...
/* 获取当前thread_pool中的线程是否满载, 如果满载则返回1, 未满载范围0 */
int thpool_thread_workload_full(threadpool pPool)
{
	int res = 0;
	pthread_mutex_lock(&(pPool->thcount_lock));
	if (pPool->num_threads_working == pPool->num_threads_alive)
	{
		res = 1;
	}
	else
	...
	pthread_mutex_unlock(&(pPool->thcount_lock));
...
struct thpool_* thpool_init(int num_threads){
	...
	if (thpool_p==NULL){
		log_error("Could not allocate memory for thread pool.");
		exit(1);
	}
	...
	if (thpool_p->threads==NULL){
		log_error("Could not allocate memory for threads.");
		exit(1);
	}
	...
	for (n=0; n<num_threads; n++){
		...
		if (THPOOL_DEBUG)
			log_debug("Created thread %d in pool.", n);
	}
...
int thpool_add_work(thpool_* thpool_p, void *(*function_p)(void*), void* arg_p){
	...
	if (newjob==NULL){
		log_error("Could not allocate memory for new job.");
		return -1;
	}
...
static void thread_init (thpool_* thpool_p, struct thread** thread_p, int id){
	...
	*thread_p = (struct thread*)malloc(sizeof(struct thread));
	if (thread_p == NULL){
		log_error("Could not allocate memory for thread.");
		exit(1);
	}
	...
	log_debug("thread id:%u", (*thread_p)->pthread);
...
static void* thread_do(struct thread* thread_p){
	...
	if (sigaction(SIGUSR1, &act, NULL) == -1) {
		log_error("Cannot handle SIGUSR1.");
	}
...
static void bsem_init(bsem *bsem_p, int value) {
	if (value < 0 || value > 1) {
		log_error("Binary semaphore can take only values 1 or 0.");
		exit(1);
	}
