...
static pthread_mutex_t xml_load_buffer_mutex;
...
void *xml_realloc(void *ptr, size_t size)
{
	...
	if (pTmp == NULL)
	{
		printf("xml_realloc failed, ptr:%p, size:%Zd", ptr, size);
	}
...
static char* xml_feed( XMLBUF *xml, int (*test)(char) )
{
	...
	while( test(xml->buf[offset]) )
	{
		...
		if(offset >= xml->len)
		{
			...
			tmp = xml_realloc(ret, size + delta + 1);
			if(!tmp)
			{
				goto xml_feed_malloc;
			}
			ret = tmp;
...
void xml_load_buffer_init()
{
	pthread_mutexattr_t mutexattr;
	pthread_mutexattr_init(&mutexattr);
	pthread_mutexattr_settype(&mutexattr, PTHREAD_MUTEX_RECURSIVE);
	pthread_mutex_init(&xml_load_buffer_mutex,&mutexattr);	

	pthread_mutexattr_destroy(&mutexattr);
}

void xml_load_buffer_destroy()
{
	pthread_mutex_destroy(&xml_load_buffer_mutex);
}
...
XmlNode* xml_load_buffer(const char * buffer, unsigned int buffer_len)
{
	...
	/* 为xml_load_buffer增加全局锁操作 */
    pthread_mutex_lock(&xml_load_buffer_mutex);
	...
	free(xml.buf);
xml_load_fail_malloc_buf:
	pthread_mutex_unlock(&xml_load_buffer_mutex);
	return ret;
}
