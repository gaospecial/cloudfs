	...
	if (tmp2 != NULL) {
		...
		for (i = 0; i < part_counts; i++) {
			*(parts + i) = part_summary_initialize();
			//经过分析, 此处atoi是OK的
			//函数原型为void (*set_part_number)(oss_multipart_object_group_t *group, int part_number);
			...
			//此处需要使用atol
			//函数原型 void (*set_size)(oss_object_summary_t *summary, long size);
		...
		unsigned short *retcode)
{
	...
	//url中的key部分需要进行url转义编码, 因此其长度要按照原始长度的6倍来准备字符串
	...
	//特别关注, metadata中此类get_xxxxx接口的内存如果返回的是字符串, 是需要调用者释放返回的字符串内存的
	//否则会导致内存泄露!!!!!!
	if (metadata != NULL) {
